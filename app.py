...
if platform.system() == "Windows":
    caminho_win = r"C:\Program Files\wkhtmltopdf\bin\wkhtmltopdf.exe"
    if os.path.exists(caminho_win):
        WKHTMLTOPDF_PATH = caminho_win
        config_pdf = pdfkit.configuration(wkhtmltopdf=WKHTMLTOPDF_PATH)
else:
    caminho_linux = shutil.which("wkhtmltopdf")
    if caminho_linux:
        WKHTMLTOPDF_PATH = caminho_linux
        config_pdf = pdfkit.configuration(wkhtmltopdf=WKHTMLTOPDF_PATH)
